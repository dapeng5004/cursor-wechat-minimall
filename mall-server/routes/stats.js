  ...
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT g.id, g.name, g.image, g.price, g.sales,
              COALESCE(SUM(og.quantity), 0) as total_sales,
              COALESCE(SUM(og.subtotal), 0) as total_amount
       FROM goods g
       LEFT JOIN order_goods og ON g.id = og.goods_id
       LEFT JOIN orders o ON og.order_id = o.id AND o.status IN (1,2,3)
       GROUP BY g.id, g.name, g.image, g.price, g.sales
